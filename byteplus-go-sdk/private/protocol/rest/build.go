...
// Whether the byte value can be sent without escaping in BYTEPLUS URLs
...
func init() {
	for i := 0; i < len(noEscape); i++ {
		// BYTEPLUS expects every character except these to be escaped
		noEscape[i] = (i >= 'A' && i <= 'Z') ||
			(i >= 'a' && i <= 'z') ||
			(i >= '0' && i <= '9') ||
			i == '-' ||
...
func buildLocationElements(r *request.Request, v reflect.Value, buildGETQuery bool) {
	...
	if !byteplus.BoolValue(r.Config.DisableRestProtocolURICleaning) {
...
func buildHeader(header *http.Header, v reflect.Value, name string, tag reflect.StructTag) error {
	...
	if err == errValueNotSet {
		...
		return bytepluserr.New(request.ErrCodeSerialization, "failed to encode REST request", err)
	}
