...
// value for chaining.
func WithRetryer(cfg *byteplus.Config, retryer Retryer) *byteplus.Config {
	cfg.Retryer = retryer
	return cfg
}
...
func isNestedErrorRetryable(parentErr bytepluserr.Error) bool {
	if parentErr == nil {
		return false
	}
...
func shouldRetryError(origErr error) bool {
	...
	case nil:
		// `bytepluserr.Error.OrigErr()` can be nil, meaning there was an error but
...
func IsErrorThrottle(err error) bool {
	if aerr, ok := err.(bytepluserr.Error); ok && aerr != nil {
