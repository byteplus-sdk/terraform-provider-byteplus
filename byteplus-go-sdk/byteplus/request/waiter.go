...
func WithWaiterLogger(logger byteplus.Logger) WaiterOption {
	return func(w *Waiter) {
		w.Logger = logger
	}
}
...
type Waiter struct {
	...
	SleepWithContext func(byteplus.Context, time.Duration) error
}
...
// Use byteplus.BackgroundContext if no context is available.
//
...
func (w Waiter) WaitWithContext(ctx byteplus.Context) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if sleepFn := req.Config.SleepDelay; sleepFn != nil {
			...
			if sleepCtxFn == nil {
				sleepCtxFn = byteplus.SleepWithContext
			}

			if err := sleepCtxFn(ctx, delay); err != nil {
				return bytepluserr.New(CanceledErrorCode, "waiter context canceled", err)
			}
		}
	}

	return bytepluserr.New(WaiterResourceNotReadyErrorCode, "exceeded wait attempts", nil)
}
...
func (a *WaiterAcceptor) match(name string, l byteplus.Logger, req *Request, err error) (bool, error) {
	result := false
	var vals []interface{}
	...
	case PathAllWaiterMatch, PathWaiterMatch:
		...
		vals, _ = byteplusutil.ValuesAtPath(req.Data, a.Argument)
		if len(vals) == 0 {
			break
		}
		result = true
		for _, val := range vals {
			if !byteplusutil.DeepEqual(val, a.Expected) {
				result = false
				break
			}
		}
	...
	case ErrorWaiterMatch:
		if aerr, ok := err.(bytepluserr.Error); ok {
	...
	case FailureWaiterState:
		...
		return true, bytepluserr.New(WaiterResourceNotReadyErrorCode,
...
func waiterLogf(logger byteplus.Logger, msg string, args ...interface{}) {
	if logger != nil {
