...
func (p *Pagination) HasNextPage() bool {
	...
	if p.EndPageOnSameToken {
		return hasNextPage && !byteplusutil.DeepEqual(p.nextTokens, p.prevTokens)
	}
	return hasNextPage
}
...
func (p *Pagination) Next() bool {
	...
	if p.started {
		for i, intok := range req.Operation.InputTokens {
			byteplusutil.SetValueAtPath(req.Params, intok, p.nextTokens[i])
		}
	}
	p.started = true
...
func (r *Request) nextPageTokens() []interface{} {
	...
	if r.Operation.TruncationToken != "" {
		tr, _ := byteplusutil.ValuesAtPath(r.Data, r.Operation.TruncationToken)
		if len(tr) == 0 {
			return nil
		}
	...
	for _, outToken := range r.Operation.OutputTokens {
		vs, _ := byteplusutil.ValuesAtPath(r.Data, outToken)
		if len(vs) == 0 {
		...
		switch tv := v.(type) {
		case *string:
			if len(byteplusutil.StringValue(tv)) == 0 {
				tokens = append(tokens, nil)
				continue
			}
		case string:
...
func logDeprecatedf(logger byteplus.Logger, flag *int32, msg string) {
	if logger == nil {
		return
	}
...
func (r *Request) NextPage() *Request {
	...
	nr := New(r.Config, r.ClientInfo, r.Handlers, r.Retryer, r.Operation, byteplusutil.CopyOf(r.Params), data)
	for i, intok := range nr.Operation.InputTokens {
		byteplusutil.SetValueAtPath(nr.Params, intok, tokens[i])
	}
	return nr
}
