...
type Config struct {
	...
	// The resolver to use for looking up endpoints for byteplus service clients
	...
	//
	//   cfg := request.WithRetryer(byteplus.NewConfig(), myRetryer)
	//
	...
	//
	// Example:
	//    sess := session.Must(session.NewSession(byteplus.NewConfig()
	...
	// the delay of a request see the byteplus/client.DefaultRetryer and
	// byteplus/request.Retryer.
	//
	...
	//         DisableRestProtocolURICleaning: byteplus.Bool(true),
	//    }))
	//
	//    svc := s3.New(sess)
	//    out, err := svc.GetObject(&s3.GetObjectInput {
	//    	Bucket: byteplus.String("bucketname"),
	//    	Key: byteplus.String("//foo//bar//moo"),
	//    })
	...
	// Example:
	//    sess := session.Must(session.NewSession(&byteplus.Config{
	//         EnableEndpointDiscovery: byteplus.Bool(true),
	//    }))
	//
	//    svc := s3.New(sess)
	//    out, err := svc.GetObject(&s3.GetObjectInput {
...
//	svc := s3.New(sess, byteplus.NewConfig().
