...
const (
	...
	accessKeyIDKey  = `byteplus_access_key_id`     // group required
	secretAccessKey = `byteplus_secret_access_key` // group required
	sessionTokenKey = `byteplus_session_token`     // optional
...
type sharedConfig struct {
	// Credentials values from the config file. Both byteplus_access_key_id and
	// byteplus_secret_access_key must be provided together in the same file to be
	// considered valid. The values will be ignored if not a complete group.
	// byteplus_session_token is an optional field that can be provided if both of
	...
	// Region is the region the SDK should use for looking up BYTEPLUS service
...
func loadSharedConfigIniFiles(filenames []string) ([]sharedConfigFile, error) {
	...
	for _, filename := range filenames {
		...
		if aerr, ok := err.(bytepluserr.Error); ok && aerr.Code() == ini.ErrCodeUnableToReadFile {
...
// example if a config file only includes byteplus_access_key_id but no
// byteplus_secret_access_key the byteplus_access_key_id will be ignored.
...
func (e SharedConfigLoadError) Error() string {
	return bytepluserr.SprintError(e.Code(), e.Message(), "", e.Err)
}
...
func (e SharedConfigAssumeRoleError) Error() string {
	return bytepluserr.SprintError(e.Code(), e.Message(), "", nil)
}
