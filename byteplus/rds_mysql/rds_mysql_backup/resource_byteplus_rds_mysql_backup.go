...
func ResourceByteplusRdsMysqlBackup() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusRdsMysqlBackupCreate,
		Read:   resourceByteplusRdsMysqlBackupRead,
		Delete: resourceByteplusRdsMysqlBackupDelete,
...
func resourceByteplusRdsMysqlBackupCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewRdsMysqlBackupService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Create(service, d, ResourceByteplusRdsMysqlBackup())
	if err != nil {
		return fmt.Errorf("error on creating rds_mysql_backup %q, %s", d.Id(), err)
	}
	return resourceByteplusRdsMysqlBackupRead(d, meta)
}

func resourceByteplusRdsMysqlBackupRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewRdsMysqlBackupService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Read(service, d, ResourceByteplusRdsMysqlBackup())
	if err != nil {
...
func resourceByteplusRdsMysqlBackupDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewRdsMysqlBackupService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Delete(service, d, ResourceByteplusRdsMysqlBackup())
	if err != nil {
