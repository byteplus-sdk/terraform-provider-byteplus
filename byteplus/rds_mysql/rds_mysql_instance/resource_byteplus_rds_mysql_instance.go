...
```
$ terraform import byteplus_rds_mysql_instance.default mysql-72da4258c2c7
```
...
func ResourceByteplusRdsMysqlInstance() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusRdsMysqlInstanceCreate,
		Read:   resourceByteplusRdsMysqlInstanceRead,
		Update: resourceByteplusRdsMysqlInstanceUpdate,
		Delete: resourceByteplusRdsMysqlInstanceDelete,
					...
					Schema: map[string]*schema.Schema{
						"charge_type": {
							...
							Description: "Payment type. Value:\nPostPaid - Pay-As-You-Go\nPrePaid - Yearly and monthly (default). \n" +
								"When the value of this field is `PrePaid`, the mysql instance cannot be deleted through terraform. Please unsubscribe the instance from the Byteplus console first, and then use `terraform state rm byteplus_rds_mysql_instance.resource_name` command to remove it from terraform state file and management.",
						},
						"auto_renew": {
	...
	dataSource := DataSourceByteplusRdsMysqlInstances().Schema["rds_mysql_instances"].Elem.(*schema.Resource).Schema
	delete(dataSource, "id")
	bp.MergeDateSourceToResource(dataSource, &resource.Schema)
	return resource
}

func resourceByteplusRdsMysqlInstanceCreate(d *schema.ResourceData, meta interface{}) (err error) {
	rdsMysqlInstanceService := NewRdsMysqlInstanceService(meta.(*bp.SdkClient))
	err = rdsMysqlInstanceService.Dispatcher.Create(rdsMysqlInstanceService, d, ResourceByteplusRdsMysqlInstance())
	if err != nil {
		return fmt.Errorf("error on creating RDS mysql instance %q, %w", d.Id(), err)
	}
	return resourceByteplusRdsMysqlInstanceRead(d, meta)
}

func resourceByteplusRdsMysqlInstanceRead(d *schema.ResourceData, meta interface{}) (err error) {
	rdsMysqlInstanceService := NewRdsMysqlInstanceService(meta.(*bp.SdkClient))
	err = rdsMysqlInstanceService.Dispatcher.Read(rdsMysqlInstanceService, d, ResourceByteplusRdsMysqlInstance())
	if err != nil {
...
func resourceByteplusRdsMysqlInstanceUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	rdsMysqlInstanceService := NewRdsMysqlInstanceService(meta.(*bp.SdkClient))
	err = rdsMysqlInstanceService.Dispatcher.Update(rdsMysqlInstanceService, d, ResourceByteplusRdsMysqlInstance())
	if err != nil {
...
func resourceByteplusRdsMysqlInstanceDelete(d *schema.ResourceData, meta interface{}) (err error) {
	rdsMysqlInstanceService := NewRdsMysqlInstanceService(meta.(*bp.SdkClient))
	err = rdsMysqlInstanceService.Dispatcher.Delete(rdsMysqlInstanceService, d, ResourceByteplusRdsMysqlInstance())
	if err != nil {
