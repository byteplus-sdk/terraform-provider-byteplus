...
type ByteplusNodePoolService struct {
	Client               *bp.SdkClient
	securityGroupService *security_group.ByteplusSecurityGroupService
}

func NewNodePoolService(c *bp.SdkClient) *ByteplusNodePoolService {
	return &ByteplusNodePoolService{
		Client:               c,
		securityGroupService: security_group.NewSecurityGroupService(c),
	}
}

func (s *ByteplusNodePoolService) GetClient() *bp.SdkClient {
	return s.Client
}

func (s *ByteplusNodePoolService) ReadResources(m map[string]interface{}) (data []interface{}, err error) {
	var (
...
func (s *ByteplusNodePoolService) ReadResource(resourceData *schema.ResourceData, nodePoolId string) (data map[string]interface{}, err error) {
	var (
		results interface{}
		resp    *map[string]interface{}
...
func (s *ByteplusNodePoolService) RefreshResourceState(resourceData *schema.ResourceData, target []string, timeout time.Duration, id string) *resource.StateChangeConf {
...
func (ByteplusNodePoolService) WithResourceResponseHandlers(nodePool map[string]interface{}) []bp.ResourceResponseHandler {
...
func (s *ByteplusNodePoolService) CreateResource(resourceData *schema.ResourceData, resource *schema.Resource) []bp.Callback {
...
func (s *ByteplusNodePoolService) ModifyResource(resourceData *schema.ResourceData, resource *schema.Resource) []bp.Callback {
			...
			Call: bp.SdkCall{
				...
				AfterRefresh: func(d *schema.ResourceData, client *bp.SdkClient, call bp.SdkCall) error {
					...
					if int(nodes["TotalCount"].(float64)) != d.Get("auto_scaling.0.desired_replicas").(int) {
						return fmt.Errorf("The number of nodes in node_pool %s is inconsistent. Suggest obtaining more detailed error message through the Byteplus console. ", d.Id())
					}
					return nil
				},
			},
...
func (s *ByteplusNodePoolService) RemoveResource(resourceData *schema.ResourceData, r *schema.Resource) []bp.Callback {
	callback := bp.Callback{
		Call: bp.SdkCall{
...
func (s *ByteplusNodePoolService) DatasourceResources(*schema.ResourceData, *schema.Resource) bp.DataSourceInfo {
...
func (s *ByteplusNodePoolService) ReadResourceId(id string) string {
	return id
}

func (s *ByteplusNodePoolService) setResourceTags(resourceData *schema.ResourceData, resourceType string, callbacks []bp.Callback) []bp.Callback {
