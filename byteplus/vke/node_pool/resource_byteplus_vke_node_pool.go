...
```
$ terraform import byteplus_vke_node_pool.default pcabe57vqtofgrbln3dp0
```

*/

func ResourceByteplusNodePool() *schema.Resource {
	return &schema.Resource{
		Create: resourceByteplusNodePoolCreate,
		Read:   resourceByteplusNodePoolRead,
		Update: resourceByteplusNodePoolUpdate,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"instance_type_ids": {
							...
							Description: "The InstanceTypeIds of NodeConfig. The value can get from byteplus_vke_support_resource_types datasource.",
						},
						"subnet_ids": {
							Type:     schema.TypeList,
							Required: true,
...
func resourceByteplusNodePoolCreate(d *schema.ResourceData, meta interface{}) (err error) {
	nodePoolService := NewNodePoolService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(nodePoolService, d, ResourceByteplusNodePool())
	if err != nil {
		return fmt.Errorf("error on creating nodePoolService  %q, %w", d.Id(), err)
	}
	return resourceByteplusNodePoolRead(d, meta)
}

func resourceByteplusNodePoolRead(d *schema.ResourceData, meta interface{}) (err error) {
	nodePoolService := NewNodePoolService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(nodePoolService, d, ResourceByteplusNodePool())
	if err != nil {
...
func resourceByteplusNodePoolUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	nodePoolService := NewNodePoolService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(nodePoolService, d, ResourceByteplusNodePool())
	if err != nil {
...
func resourceByteplusNodePoolDelete(d *schema.ResourceData, meta interface{}) (err error) {
	nodePoolService := NewNodePoolService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(nodePoolService, d, ResourceByteplusNodePool())
	if err != nil {
