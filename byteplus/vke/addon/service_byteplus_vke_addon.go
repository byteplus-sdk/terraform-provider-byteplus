...
type ByteplusVkeAddonService struct {
	Client *bp.SdkClient
}

func NewVkeAddonService(c *bp.SdkClient) *ByteplusVkeAddonService {
	return &ByteplusVkeAddonService{
		Client: c,
	}
}

func (s *ByteplusVkeAddonService) GetClient() *bp.SdkClient {
	return s.Client
}

func (s *ByteplusVkeAddonService) ReadResources(condition map[string]interface{}) (data []interface{}, err error) {
	var (
...
func (s *ByteplusVkeAddonService) ReadResource(resourceData *schema.ResourceData, resourceId string) (data map[string]interface{}, err error) {
	var (
		results []interface{}
		ok      bool
	)
...
func (s *ByteplusVkeAddonService) RefreshResourceState(resourceData *schema.ResourceData, target []string, timeout time.Duration, id string) *resource.StateChangeConf {
...
func (ByteplusVkeAddonService) WithResourceResponseHandlers(data map[string]interface{}) []bp.ResourceResponseHandler {
	return []bp.ResourceResponseHandler{}
}

func (s *ByteplusVkeAddonService) CreateResource(resourceData *schema.ResourceData, resource *schema.Resource) []bp.Callback {
...
func (s *ByteplusVkeAddonService) ModifyResource(resourceData *schema.ResourceData, resource *schema.Resource) []bp.Callback {
...
func (s *ByteplusVkeAddonService) RemoveResource(resourceData *schema.ResourceData, r *schema.Resource) []bp.Callback {
	callback := bp.Callback{
		Call: bp.SdkCall{
			Action:      "DeleteAddon",
			ContentType: bp.ContentTypeJson,
				...
				if strings.Contains(baseErr.Error(), "forbidden to delete required addon") { // 一些组件禁止删除，直接返回
					msg := fmt.Sprintf("error: %s. msg: %s",
						...
						"If you want to remove it form terraform state, "+
							"please use `terraform state rm byteplus_vke_addon.resource_name` command ")
...
func (s *ByteplusVkeAddonService) DatasourceResources(*schema.ResourceData, *schema.Resource) bp.DataSourceInfo {
...
func (s *ByteplusVkeAddonService) ReadResourceId(id string) string {
	return id
}

func (s *ByteplusVkeAddonService) updateConfig(name interface{}, data map[string]interface{}, client *bp.SdkClient, call bp.SdkCall) (*map[string]interface{}, error) {
	var (
