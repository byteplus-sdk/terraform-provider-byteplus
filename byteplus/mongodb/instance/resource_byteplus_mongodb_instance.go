...
func ResourceByteplusMongoDBInstance() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusMongoDBInstanceCreate,
		Read:   resourceByteplusMongoDBInstanceRead,
		Update: resourceByteplusMongoDBInstanceUpdate,
		Delete: resourceByteplusMongoDBInstanceDelete,
		...
		Schema: map[string]*schema.Schema{
			...
			"db_engine_version": {
				...
				Computed:    true,
				ForceNew:    true,
				Description: "The version of db engine, valid value contains `MongoDB_4_2`, `MongoDB_4_2`, `MongoDB_4_4`, `MongoDB_5_0`, `MongoDB_6_0`.",
			},
			"node_spec": {
				Type:        schema.TypeString,
				Required:    true,
...
func resourceByteplusMongoDBInstanceCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewMongoDBInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(service, d, ResourceByteplusMongoDBInstance())
	if err != nil {
		return fmt.Errorf("Error on creating instance %q,%s", d.Id(), err)
	}
	return resourceByteplusMongoDBInstanceRead(d, meta)
}

func resourceByteplusMongoDBInstanceUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewMongoDBInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(service, d, ResourceByteplusMongoDBInstance())
	if err != nil {
...
func resourceByteplusMongoDBInstanceDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewMongoDBInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(service, d, ResourceByteplusMongoDBInstance())
	if err != nil {
...
func resourceByteplusMongoDBInstanceRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewMongoDBInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(service, d, ResourceByteplusMongoDBInstance())
	if err != nil {
