...
func DataSourceByteplusMongoDBInstances() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceByteplusMongoDBInstancesRead,
		Schema: map[string]*schema.Schema{
			...
			"db_engine_version": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validation.StringInSlice([]string{"MongoDB_4_2"}, false),
				Description:  "The version of db engine to query, valid value contains `MongoDB_4_2`.",
			},
			"create_start_time": {
				Type:        schema.TypeString,
				Optional:    true,
...
func dataSourceByteplusMongoDBInstancesRead(d *schema.ResourceData, meta interface{}) error {
	service := NewMongoDBInstanceService(meta.(*bp.SdkClient))
	return bp.DefaultDispatcher().Data(service, d, DataSourceByteplusMongoDBInstances())
}
