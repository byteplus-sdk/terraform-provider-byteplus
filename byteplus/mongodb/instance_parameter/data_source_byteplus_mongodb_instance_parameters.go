...
func DataSourceByteplusMongoDBInstanceParameters() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceByteplusMongoDBInstanceParametersRead,
...
func dataSourceByteplusMongoDBInstanceParametersRead(d *schema.ResourceData, meta interface{}) error {
	service := NewMongoDBInstanceParameterService(meta.(*bp.SdkClient))
	err := bp.DefaultDispatcher().Data(service, d, DataSourceByteplusMongoDBInstanceParameters())
	if err != nil {
		return err
	}
	...
	resource := DataSourceByteplusMongoDBInstanceParameters()
	dataSourceInfo := service.DatasourceResources(d, resource)
	dataSourceInfo.CollectField = "parameters"
	dataSourceInfo.ResponseConverts = map[string]bp.ResponseConvert{
		"DBEngine": {
	...
	return bp.ResponseToDataSource(d, resource, dataSourceInfo, results)
}
