...
func ResourceByteplusAlb() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusAlbCreate,
		Read:   resourceByteplusAlbRead,
		Update: resourceByteplusAlbUpdate,
		Delete: resourceByteplusAlbDelete,
		...
		Schema: map[string]*schema.Schema{
			...
			"eip_billing_config": {
				...
				Description: "The billing configuration of the EIP which automatically associated to the Alb. This field is valid when the type of the Alb is `public`." +
					"When the type of the Alb is `private`, suggest using a combination of resource `byteplus_eip_address` and `byteplus_eip_associate` to achieve public network access function.",
			...
			"ipv6_eip_billing_config": {
				...
				Description: "The billing configuration of the Ipv6 EIP which automatically associated to the Alb. This field is required when the type of the Alb is `public`." +
					"When the type of the Alb is `private`, suggest using a combination of resource `byteplus_vpc_ipv6_gateway` and `byteplus_vpc_ipv6_address_bandwidth` to achieve ipv6 public network access function.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"isp": {
							Type:         schema.TypeString,
...
func resourceByteplusAlbCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Create(service, d, ResourceByteplusAlb())
	if err != nil {
		return fmt.Errorf("error on creating alb %q, %s", d.Id(), err)
	}
	return resourceByteplusAlbRead(d, meta)
}

func resourceByteplusAlbRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Read(service, d, ResourceByteplusAlb())
	if err != nil {
...
func resourceByteplusAlbUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Update(service, d, ResourceByteplusAlb())
	if err != nil {
...
func resourceByteplusAlbDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Delete(service, d, ResourceByteplusAlb())
	if err != nil {
