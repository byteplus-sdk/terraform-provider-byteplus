...
func ResourceByteplusAlbListener() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusAlbListenerCreate,
		Read:   resourceByteplusAlbListenerRead,
		Update: resourceByteplusAlbListenerUpdate,
		Delete: resourceByteplusAlbListenerDelete,
...
func resourceByteplusAlbListenerCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbListenerService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Create(service, d, ResourceByteplusAlbListener())
	if err != nil {
		return fmt.Errorf("error on creating alb_listener %q, %s", d.Id(), err)
	}
	return resourceByteplusAlbListenerRead(d, meta)
}

func resourceByteplusAlbListenerRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbListenerService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Read(service, d, ResourceByteplusAlbListener())
	if err != nil {
...
func resourceByteplusAlbListenerUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbListenerService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Update(service, d, ResourceByteplusAlbListener())
	if err != nil {
...
func resourceByteplusAlbListenerDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewAlbListenerService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Delete(service, d, ResourceByteplusAlbListener())
	if err != nil {
