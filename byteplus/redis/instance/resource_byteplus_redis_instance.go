...
```
$ terraform import byteplus_redis_instance.default redis-n769ewmjjqyqh5dv
```
...
func ResourceByteplusRedisDbInstance() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusRedisDbInstanceCreate,
		Read:   resourceByteplusRedisDbInstanceRead,
		Update: resourceByteplusRedisDbInstanceUpdate,
		Delete: resourceByteplusRedisDbInstanceDelete,
...
func resourceByteplusRedisDbInstanceCreate(d *schema.ResourceData, meta interface{}) (err error) {
	redisInstanceService := NewRedisDbInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(redisInstanceService, d, ResourceByteplusRedisDbInstance())
	if err != nil {
		return fmt.Errorf("Error on creating instance %q,%s", d.Id(), err)
	}
	return resourceByteplusRedisDbInstanceRead(d, meta)
}

func resourceByteplusRedisDbInstanceUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	redisInstanceService := NewRedisDbInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(redisInstanceService, d, ResourceByteplusRedisDbInstance())
	if err != nil {
...
func resourceByteplusRedisDbInstanceDelete(d *schema.ResourceData, meta interface{}) (err error) {
	redisInstanceService := NewRedisDbInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(redisInstanceService, d, ResourceByteplusRedisDbInstance())
	if err != nil {
...
func resourceByteplusRedisDbInstanceRead(d *schema.ResourceData, meta interface{}) (err error) {
	redisInstanceService := NewRedisDbInstanceService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(redisInstanceService, d, ResourceByteplusRedisDbInstance())
	if err != nil {
