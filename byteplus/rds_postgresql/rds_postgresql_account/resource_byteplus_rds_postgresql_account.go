...
func ResourceByteplusRdsPostgresqlAccount() *schema.Resource {
	return &schema.Resource{
		Create: resourceByteplusRdsPostgresqlAccountCreate,
		Read:   resourceByteplusRdsPostgresqlAccountRead,
		Update: resourceByteplusRdsPostgresqlAccountUpdate,
		Delete: resourceByteplusRdsPostgresqlAccountDelete,
...
func resourceByteplusRdsPostgresqlAccountCreate(d *schema.ResourceData, meta interface{}) (err error) {
	rdsAccountService := NewRdsPostgresqlAccountService(meta.(*bp.SdkClient))
	err = rdsAccountService.Dispatcher.Create(rdsAccountService, d, ResourceByteplusRdsPostgresqlAccount())
	if err != nil {
		return fmt.Errorf("error on creating rds postgresql account %q, %w", d.Id(), err)
	}
	return resourceByteplusRdsPostgresqlAccountRead(d, meta)
}

func resourceByteplusRdsPostgresqlAccountRead(d *schema.ResourceData, meta interface{}) (err error) {
	rdsAccountService := NewRdsPostgresqlAccountService(meta.(*bp.SdkClient))
	err = rdsAccountService.Dispatcher.Read(rdsAccountService, d, ResourceByteplusRdsPostgresqlAccount())
	if err != nil {
...
func resourceByteplusRdsPostgresqlAccountUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewRdsPostgresqlAccountService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Update(service, d, ResourceByteplusRdsPostgresqlAccount())
	if err != nil {
...
func resourceByteplusRdsPostgresqlAccountDelete(d *schema.ResourceData, meta interface{}) (err error) {
	rdsAccountService := NewRdsPostgresqlAccountService(meta.(*bp.SdkClient))
	err = rdsAccountService.Dispatcher.Delete(rdsAccountService, d, ResourceByteplusRdsPostgresqlAccount())
	if err != nil {
