...
func ResourceByteplusScalingConfiguration() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusScalingConfigurationCreate,
		Read:   resourceByteplusScalingConfigurationRead,
		Update: resourceByteplusScalingConfigurationUpdate,
		Delete: resourceByteplusScalingConfigurationDelete,
	...
	dataSource := DataSourceByteplusScalingConfigurations().Schema["scaling_configurations"].Elem.(*schema.Resource).Schema
	delete(dataSource, "id")
	bp.MergeDateSourceToResource(dataSource, &resource.Schema)
	return resource
}

func resourceByteplusScalingConfigurationCreate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(scalingConfigurationService, d, ResourceByteplusScalingConfiguration())
	if err != nil {
		return fmt.Errorf("error on creating ScalingConfiguration %q, %s", d.Id(), err)
	}
	return resourceByteplusScalingConfigurationRead(d, meta)
}

func resourceByteplusScalingConfigurationRead(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(scalingConfigurationService, d, ResourceByteplusScalingConfiguration())
	if err != nil {
...
func resourceByteplusScalingConfigurationUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(scalingConfigurationService, d, ResourceByteplusScalingConfiguration())
	if err != nil {
...
func resourceByteplusScalingConfigurationDelete(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(scalingConfigurationService, d, ResourceByteplusScalingConfiguration())
	if err != nil {
