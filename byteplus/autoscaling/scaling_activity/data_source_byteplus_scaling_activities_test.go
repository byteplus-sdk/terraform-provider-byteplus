...
const testAccByteplusScalingActivitiesDatasourceConfig = `
...
resource "byteplus_scaling_instance_attachment" "foo" {
    ...
    scaling_group_id = "${byteplus_scaling_group_enabler.foo.scaling_group_id}"
    entrusted = true
}

locals {
  	scaling_group_ids = toset(byteplus_scaling_instance_attachment.foo[*].scaling_group_id)
}
...
func TestAccByteplusScalingActivitiesDatasource_Basic(t *testing.T) {
	resourceName := "data.byteplus_scaling_activities.foo"

	acc := &byteplus.AccTestResource{
		ResourceId: resourceName,
		Svc:        &scaling_activity.ByteplusScalingActivityService{},
	}
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccByteplusScalingActivitiesDatasourceConfig,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(acc.ResourceId, "activities.#", "2"),
				),
			},
		},
	})
