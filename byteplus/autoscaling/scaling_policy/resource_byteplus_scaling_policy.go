...
func ResourceByteplusScalingPolicy() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceByteplusScalingPolicyCreate,
		Read:   resourceByteplusScalingPolicyRead,
		Update: resourceByteplusScalingPolicyUpdate,
		Delete: resourceByteplusScalingPolicyDelete,
...
func resourceByteplusScalingPolicyCreate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingPolicyService := NewScalingPolicyService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(scalingPolicyService, d, ResourceByteplusScalingPolicy())
	if err != nil {
		return fmt.Errorf("error on creating ScalingPolicy %q, %s", d.Id(), err)
	}
	return resourceByteplusScalingPolicyRead(d, meta)
}

func resourceByteplusScalingPolicyRead(d *schema.ResourceData, meta interface{}) (err error) {
	scalingPolicyService := NewScalingPolicyService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(scalingPolicyService, d, ResourceByteplusScalingPolicy())
	if err != nil {
...
func resourceByteplusScalingPolicyUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingPolicyService := NewScalingPolicyService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(scalingPolicyService, d, ResourceByteplusScalingPolicy())
	if err != nil {
...
func resourceByteplusScalingPolicyDelete(d *schema.ResourceData, meta interface{}) (err error) {
	scalingPolicyService := NewScalingPolicyService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(scalingPolicyService, d, ResourceByteplusScalingPolicy())
	if err != nil {
