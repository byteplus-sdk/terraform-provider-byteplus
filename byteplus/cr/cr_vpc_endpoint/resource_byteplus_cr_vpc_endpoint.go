...
```
$ terraform import byteplus_cr_vpc_endpoint.default crVpcEndpoint:cr-basic
```

*/

func ResourceByteplusCrVpcEndpoint() *schema.Resource {
	resource := &schema.Resource{
		Read:   resourceByteplusCrVpcEndpointRead,
		Create: resourceByteplusCrVpcEndpointCreate,
		Update: resourceByteplusCrVpcEndpointUpdate,
		Delete: resourceByteplusCrVpcEndpointDelete,
	...
	dataSource := DataSourceByteplusCrVpcEndpoints().Schema["endpoints"].Elem.(*schema.Resource).Schema
	bp.MergeDateSourceToResource(dataSource, &resource.Schema)
	return resource
}

func resourceByteplusCrVpcEndpointCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewCrVpcEndpointService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(service, d, ResourceByteplusCrVpcEndpoint())
	if err != nil {
		return fmt.Errorf("error on creating CrVpcEndpoint %q, %s", d.Id(), err)
	}
	return resourceByteplusCrVpcEndpointRead(d, meta)
}

func resourceByteplusCrVpcEndpointUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewCrVpcEndpointService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(service, d, ResourceByteplusCrVpcEndpoint())
	if err != nil {
...
func resourceByteplusCrVpcEndpointDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewCrVpcEndpointService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(service, d, ResourceByteplusCrVpcEndpoint())
	if err != nil {
...
func resourceByteplusCrVpcEndpointRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewCrVpcEndpointService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(service, d, ResourceByteplusCrVpcEndpoint())
	if err != nil {
