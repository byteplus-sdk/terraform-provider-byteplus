...
func DataSourceByteplusCrRegistries() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceByteplusCrRegistriesRead,
			...
			"registries": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"resource_tags": bp.TagsSchemaComputed(),
						"status": {
							Type:        schema.TypeList,
							MaxItems:    1,
							Computed:    true,
...
func dataSourceByteplusCrRegistriesRead(d *schema.ResourceData, meta interface{}) error {
	service := NewCrRegistryService(meta.(*bp.SdkClient))
	return bp.DefaultDispatcher().Data(service, d, DataSourceByteplusCrRegistries())
}
