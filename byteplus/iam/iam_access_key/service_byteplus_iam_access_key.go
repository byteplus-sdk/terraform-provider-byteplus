...
type ByteplusIamAccessKeyService struct {
	Client *bp.SdkClient
}

func NewIamAccessKeyService(c *bp.SdkClient) *ByteplusIamAccessKeyService {
	return &ByteplusIamAccessKeyService{
		Client: c,
	}
}
		...
		Call: bp.SdkCall{
			...
			AfterCall: func(d *schema.ResourceData, client *bp.SdkClient, resp *map[string]interface{}, call bp.SdkCall) error {
				...
				if v, ok := d.GetOk("pgp_key"); ok && len(v.(string)) > 0 {
					...
					fingerprint, encrypted, err := encryption.EncryptValue(encryptionKey, sk.(string), "Byteplus IAM Access Key Secret")
					if err != nil {
...
func (s *ByteplusIamAccessKeyService) updateAccessKeyStatus(status string, resourceData *schema.ResourceData) bp.Callback {
	return bp.Callback{
		Call: bp.SdkCall{
