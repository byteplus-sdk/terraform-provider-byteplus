...
func ResourceByteplusClb() *schema.Resource {
	return &schema.Resource{
		Create: resourceByteplusClbCreate,
		Read:   resourceByteplusClbRead,
		Update: resourceByteplusClbUpdate,
		Delete: resourceByteplusClbDelete,
		...
		Schema: map[string]*schema.Schema{
			...
			"eip_billing_config": {
				...
				Description: "The billing configuration of the EIP which automatically associated to CLB. This field is valid when the type of CLB is `public`." +
					"When the type of the CLB is `private`, suggest using a combination of resource `byteplus_eip_address` and `byteplus_eip_associate` to achieve public network access function.",
			...
			"address_ip_version": {
				...
				Description: "The address ip version of the Clb. Valid values: `ipv4`, `DualStack`. Default is `ipv4`.\n" +
					...
					"and suggest using a combination of resource `byteplus_vpc_ipv6_gateway` and `byteplus_vpc_ipv6_address_bandwidth` to achieve ipv6 public network access function.",
			},
			"eni_ipv6_address": {
				Type:         schema.TypeString,
				Optional:     true,
...
func resourceByteplusClbCreate(d *schema.ResourceData, meta interface{}) (err error) {
	clbService := NewClbService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Create(clbService, d, ResourceByteplusClb())
	if err != nil {
		return fmt.Errorf("error on creating clb  %q, %w", d.Id(), err)
	}
	return resourceByteplusClbRead(d, meta)
}

func resourceByteplusClbRead(d *schema.ResourceData, meta interface{}) (err error) {
	clbService := NewClbService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Read(clbService, d, ResourceByteplusClb())
	if err != nil {
...
func resourceByteplusClbUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	clbService := NewClbService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Update(clbService, d, ResourceByteplusClb())
	if err != nil {
...
func resourceByteplusClbDelete(d *schema.ResourceData, meta interface{}) (err error) {
	clbService := NewClbService(meta.(*bp.SdkClient))
	err = bp.DefaultDispatcher().Delete(clbService, d, ResourceByteplusClb())
	if err != nil {
